...
4. **Reconcile Impact** — this is the core of the agent. The `**Impact**` block is a markdown **table** with 4 columns (`Direction` / `Surface` / `Kind` / `Note`) and a closed set of 5 `Direction` values (`Add` / `Update` / `Delete` / `Contradict` / `Downstream`). Walk the rows:
   ...
   - If the `**Impact**` table is present but contains **zero `Downstream` rows** AND the `## Plan Sketch`'s `**Constraints**` block does not contain a line with the fixed substring `Downstream: none — confirmed by ` (em dash `—`, U+2014, matches `hq:workflow § Plan Sketch § **Impact**` Downstream check directive), emit a "missing Downstream declaration" FB at Medium severity. The directive requires the plan author to explicitly declare the zero-Downstream case via a sentinel `**Constraints**` line so silent omissions become auditable; a plan that reaches `/hq:start` Phase 6 without either a `Downstream` row or the sentinel has bypassed the draft-time prompt and must be flagged here. This is a drafting defect sibling to "missing Impact" — the Impact block exists but its zero-Downstream claim is unsubstantiated.
