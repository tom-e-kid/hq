...
2. **`## FB Analysis`** — one entry per FB file under `feedbacks/done/` at Phase 8 entry time. Entry format and the 4 categorical axes (`detection_validity` / `preventable_at_implementation` / `prevention_lever` / `notes`) are specified in `hq:workflow` § Retrospective. **Zero-FB case**: when `feedbacks/done/` has no FB files, emit the literal body `(no FBs to analyze)` under the section header. Do NOT omit the section — the primary acceptance gate counts the three section headers.
