...
- **English (fixed — auto-injected or parse-targeted)**:
  ...
  - `hq:task` / `hq:plan` prescribed headings: `## Why`, `## Approach`, `## Editable surface`, `## Plan`, `## Acceptance` (all consumed by `/hq:draft` / `/hq:start`)
  - PR body **workflow sections**: `## Primary Verification (manual)` (Phase 8 escape-hatch gate), `## Manual Verification` (Phase 8 auto-inject from `[manual]` Acceptance items), `## Known Issues` (Phase 8 auto-inject from pending FBs + `/hq:triage` literal-grep target)
...
2. **Workflow sections layer** — the sections auto-injected by `/hq:start` Phase 8: `## Primary Verification (manual)` / `## Manual Verification` / `## Known Issues` (each emitted only when its trigger condition holds), followed by the `Closes` / `Refs` trailer. These headings are **English-fixed** (each has an injection or parse contract — see § Language) and not overridable by `.hq/pr.md`.
