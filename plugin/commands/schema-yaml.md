...
`docs/schema.md` をベースに、`chunk-schema/v1` 準拠の `docs/schema.yaml` を生成する。

## 手順

1. `docs/schema.md` を読み込み、テーブル構造・リレーション情報を抽出する
2. `apps/schema-visualizer/README.md` の「Input 仕様 — chunk-schema/v1」(L29-124) をフォーマット仕様として参照する
3. プロジェクト内の Drizzle schema 定義ファイルでカラムの nullable/default 等の詳細をクロスチェックする
4. `chunk-schema/v1` フォーマットに準拠した YAML を `docs/schema.yaml` に上書き生成する

## フォーマット仕様の参照先

**`apps/schema-visualizer/README.md` の L29-124 に定義された `chunk-schema/v1` 仕様に厳密に従うこと。**
...
- schema.md のリレーション図と schema.ts の FK 定義の両方を照合して生成する
...
- schema.md の情報が schema.ts と矛盾する場合は schema.ts を正とする
- schema.md に記載がないが schema.ts に存在するテーブル/カラムがあれば追加する
