...
- **`hq:feedback`** — GitHub Issue for escalated residuals. Created only with explicit user confirmation (loop Stage 7, or `/hq:copilot`).
...
Agents (`plugin/v3/agents/`): `executor` (build; model inherit) · `code-reviewer` / `security-scanner` (sonnet) / `integrity-checker` (review) · `retro-distiller` (sonnet) · `review-comment-analyzer` (used by `/hq:copilot`).
