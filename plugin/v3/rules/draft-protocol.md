# Draft Protocol — Brainstorm & Create `hq:plan`
...
When the input argument is empty, the intake task has nothing to fetch — mark it `completed` immediately after Phase 1 finishes. The row is kept so the overall phase count stays stable.
...
3. Project Overrides: `cat .hq/draft.md 2>/dev/null || echo "none"`
...
**`hq:workflow`** — shorthand for `${CLAUDE_PLUGIN_ROOT}/plugin/v3/rules/workflow.md` (plugin-internal source of truth). Read it with the Read tool when this protocol starts so all subsequent phases have the rule available. All `hq:workflow § <name>` citations below refer to sections of that file.

## Phase 1: Intake (hq:task + pre-session context + wide-impact survey)

Three inputs feed the brainstorm:

**`hq:task` Issue (optional)** — when the input argument is provided:
...
Keep the fetched task data (title, body, milestone, labels, projects), any supplementary text from the input argument, your read of the pre-session context, and the survey outcomes in conversation state. **Do not** write any files yet — the task directory and plan file are created in Phase 4, after the user's `go`.
...
- **Invariant uniformity** — when the change adds or touches a mutation path (save / edit / delete / state write), ask: what rules do the sibling paths already enforce (validation, guards, rejection), and does this plan apply them uniformly across every path it introduces or touches? An invariant enforced on one path and silently skipped on its sibling is a recorded top source of escalate-grade review findings — decide the uniform rule here, in the plan, not at review.
- **State-transition lifecycle** — when the change introduces or modifies a state transition (a flag, a lock, a mode switch), commit its lifecycle in the plan: what triggers it (an actual value change, or any touch event?), whether a reverse path exists, and whether the transition is observable to the user. A one-way silent transition decided implicitly at build time is an escalate-grade finding by default.
