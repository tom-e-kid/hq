...
- **`hq:feedback`** — a GitHub Issue (label: `hq:feedback`) for residual problems the loop escalates. Created only with explicit user confirmation: at `/hq:loop` Stage 7 (from triage escalate-candidates), or by `/hq:copilot` / `/hq:copilot-loop` for external review comments.
...
| `.hq/copilot.md` | `/hq:copilot` / `/hq:copilot-loop` | Reply tone / language, project-specific dismissal criteria |
...
- **Worktree propagation.** `plugin/v3/skills/worktree-setup/scripts/worktree-setup.sh` symlinks the main repo's `.hq/*.md` (override files + `start-memory.md`), `.hq/retro/`, and `.hq/tasks/` into a newly created worktree, so the worktree shares the same behavior and loop write-back accumulates in the main repo. The `.md` set is globbed (not a fixed list), so new override file names need no change to that script.
...
- **`hq:feedback` creation is user-gated** at Stage 7 (and `/hq:copilot` / `/hq:copilot-loop` for external review comments) — the root never creates one alone.
- **J8 is the loop control** — semantic convergence judgment; `loop_max_iterations` (default 2) is only the runaway backstop.
- `/hq:copilot` / `/hq:copilot-loop` and `/hq:archive` remain standalone post-PR tools.
...
4. `hq:feedback` Issues are created only at Stage 7 from user-selected candidates (`Refs #<PR>`), and by `/hq:copilot` / `/hq:copilot-loop` for external review comments.
